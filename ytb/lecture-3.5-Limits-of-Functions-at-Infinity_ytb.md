...
● [0:00]. Reminder of the definition of vertical asymptote (VA): lim ₓ→ₐ 𝒇(𝒙) = ±∞.png) 
...
● [3:55]. Difference between removable discontinuities (holes) and VA. and VA.png) 
...
● [6:40]. 🧩 Example –: 𝒇(𝒙) = 𝒙 / ((𝒙 + 3)(𝒙 − 1)) = 𝒙 ÷ ((𝒙 + 3)(𝒙 − 1)).png) 
     ...
     ◉ [8:14]. Using sign analysis to determine behavior around VAs:
          ○ Sign chart:
                                                     ¦ ◟                              ¦ ◟  
                                        -----------∣--------------∣-------------∣------------
                      𝒇(𝒙)               -    ◝ ¦         +              -    ◝ ¦   +
...
● [16:30]. Question: What happens to a function as **𝒙 approaches positive or negative infinity?** 
     ◉ Exploring the behavior of 1/𝒙 as 𝒙 increases:
          ○ As 𝒙 increases, 1/𝒙 approaches zero. 
               ■ lim ₓ→∞ 1/𝒙 = 0 
               ■ lim ₓ→ -∞ 1/𝒙 = 0 
   
● [20:25]. Definition of limit at infinity : 
     ◉ If 𝒇(𝒙) approaches a number as 𝒙 approaches infinity, the limit exists.
          
● [21:07]. Relation between limits at infinity and **horizontal asymptotes (HA)**:
     ◉ HA represent the value a function approaches as 𝒙 goes to positive or negative infinity. 
          
● [23:00]. The rules for calculating limits as 𝒙 approaches positive infinity (𝒙 → +∞) or negative infinity 
...
● [26:10]. Behavior of polynomials at infinity 
...
● [38:15]. 🧩 Example –: llim ₓ→∞ (5𝒙 - 2) / (3𝒙 + 9)÷(3𝒙 + 9).png) 
...
● [44:28]. 🧩 Example –: lim ₓ→∞  (5𝒙² - 4𝒙) / (15𝒙³ - 3𝒙) ÷ (15𝒙³ - 3𝒙).png) 
...
● [48:05]. 🧩 Example –: lim ₓ→∞ (7𝒙³ - 2𝒙² + 1) / (9 - 2𝒙) ÷ (9 - 2𝒙).png) 
     ◉ Divide all terms by the highest power of x in the denominator (x):
...
● [52:15]. 🧩 Example –: llim ₓ→∞ ³√( (2𝒙² - 3) / (𝒙² - 5) ) ÷ (𝒙² - 5)].png) 
...
● [53:50]. 🧩 Example –: lim ₓ→-∞ (√(𝒙² + 2) ) / (3𝒙 - 6) ) ÷  (3𝒙 - 6)_1.png) ) ÷  (3𝒙 - 6)_2.png) ) ÷  (3𝒙 - 6)_3.png) 
     ◉ Divide each term by 𝒙, the highest power in the denominator.
          ➀  [56:00]. _Applied Property_: Multiplication of Terms with Fractional Powers: a⋅b = (a²⋅b²)¹ᐟ²
...
● [1:06:53]. 🧩 Example –: lim ₓ→∞ (√(𝒙⁴ + 2) - 𝒙²) - 𝒙²).png) 
...
● [1:16:00]. 🧩 Example –: lim ₓ→∞ (√(𝒙⁴ + 2𝒙) - 𝒙²) - 𝒙²) .png) 
     ◉ Rationalize the expression by multiplying by the conjugate. 
     ◉ Divide each term by 𝒙², the highest power in the denominator. 
     ◉ Step 1: Multiply by the conjugate to rationalize the expression
