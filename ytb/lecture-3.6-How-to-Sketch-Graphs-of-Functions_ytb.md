...
● [0:41]. Presentation of the 🧩 Example –: 𝒇(𝒙) = (𝒙 + 2)(𝒙 - 1)²  

● [1:18]. Step 1: Find the intercepts
     ◉ Find 𝒙-intercepts:
          ...
          ○ (𝒙 + 2)(𝒙 − 1)² = 0 ⇒  𝒙 + 2 = 0     →     𝒙 = −2 ;  𝒙 − 1 = 0     →     𝒙 = 1
               ■ So the x-intercepts are:   𝒙 = −2   and   𝒙 = 1
     ◉ Find the 𝑦-intercept:
          ○ Set 𝒙 = 0 and solve for 𝑦: 𝒇(0) = (0 + 2)(0 − 1)² = 2 · 1 = 2
               ■ So the y-intercept is: 𝒚 = 2
       
● [6:04]. Step 2: Find asymptotes (only for rational 𝒇(𝒙))
     ◉ Rational 𝒇(𝒙) can have vertical, horizontal asymptotes, and holes.
    ❶ ᴠ̳ᴇ̳ʀ̳ᴛ̳ɪ̳ᴄ̳ᴀ̳ʟ̳ ̳ᴀ̳ꜱ̳ʏ̳ᴍ̳ᴘ̳ᴛ̳ᴏ̳ᴛ̳ᴇ̳ꜱ̳: Arise when the function tends to ±∞ as 𝒙 approaches a specific value.
          ○ Definition:
               ■ This occurs when:
          ...
          ○ One-Sided Limits:
               ...
               ■ The function may diverge differently from each side:
                  Example: 𝒇(𝒙) = 1 / (𝒙 − a)
                     → limₓ→ₐ⁻ 𝒇(𝒙) = −∞,  limₓ→ₐ⁺ 𝒇(𝒙) = +∞
                     → limₓ→ₐ 𝒇(𝒙) D.N.E. (does not exist), but the vertical asymptote is still at x = a.
          ○ 🗒️ NOTE: 
             The existence of a vertical asymptote **does not require** the two-sided limit to diverge in the same way or to exist at all.
              What matters is that at least one of the one-sided limits diverges to ±∞.
   ❷ ʜ̳ᴏ̳ʟ̳ᴇ̳ꜱ̳ (Removable Discontinuities): Occur where the denominator equals zero, but the corresponding factor can be canceled.
          ...
          ○ 🗒️ NOTE:
            A **hole does not cause the function to diverge to ∞** like a vertical asymptote.
            Instead, it’s a “missing point” on an otherwise continuous curve, where the limit exists but the function is undefined.
  ❸ ʜ̳ᴏ̳ʀ̳ɪ̳ᴢ̳ᴏ̳ɴ̳ᴛ̳ᴀ̳ʟ̳ ̳ᴀ̳ꜱ̳ʏ̳ᴍ̳ᴘ̳ᴛ̳ᴏ̳ᴛ̳ᴇ̳ꜱ̳: Determined by evaluating the limit of a function as 𝒙 → ±∞.
          ○ Definition:
               ■ A horizontal asymptote (HA) occurs when:
                 (i) lim ₓ→+∞ 𝒇(𝒙) = 𝒍   or   lim ₓ→−∞ 𝒇(𝒙) = 𝒍  
                      where 𝒍 is a real number.  
                 (ii) The value 𝒍 represents the **end behavior** of the function — the value that 𝒇(𝒙) gets closer to  
          ...
          ○ Directional (End) Behavior:
               ■ Evaluate the limits separately as 𝒙 → +∞ and as 𝒙 → −∞:
                    ▣ lim ₓ→+∞ 𝒇(𝒙)
                    ▣ lim ₓ→−∞ 𝒇(𝒙)
               ■ These are sometimes referred to as "one-sided" limits at infinity.
                    ▣ While not the same as limits from the left/right at a point (like lim ₓ→𝒄⁺ or lim ₓ→𝒄⁻),
                       they illustrate how a function might tend to **different horizontal asymptotes** at each end of the 𝒙-axis.
          ○ 🗒️ NOTE:
            Including examples of behavior as 𝒙 → +∞ and 𝒙 → −∞ helps students understand the concept of asymptotic behavior more clearly.
               ■ It reinforces that limits at infinity don’t always match:
                    ▣ A function may level off to one value as 𝒙 → +∞, and a different one (or none) as 𝒙 → −∞.
               ■ Just like with finite limits, the concept of a two-sided limit (limₓ→𝒄 𝒇(𝒙)) **only exists if both sides agree**:
                    ▣ lim ₓ→𝒄⁻ 𝒇(𝒙) = lim ₓ→𝒄⁺ 𝒇(𝒙)
          ○ Summary:
               ■ If limₓ→±∞ 𝒇(𝒙) = 𝒍, then 𝑦 = 𝒍 is a horizontal asymptote.
...
● [9:07]. Step 3: Perform the first derivative test
...
● [10:00]. Step 4: Perform the second derivative test
...
Ａｐｐｌｙｉｎｇ　ｔｈｅ　ｓｔｅｐｓ　ｔｏ　ｔｈｅ　ｉｎｉｔｉａｌ　ｅｘａｍｐｌｅ     𝒇（𝒙）　＝　（𝒙　＋　２）（𝒙　－　１）²
...
● [13:28]. Step 3: First derivative test:
...
● [15:05]. Step 4: Second derivative test:
     ◉ 𝒇''(𝒙) = 6𝒙  
     ◉ Find the inflection point by setting 𝒇''(𝒙) = 0.
          ○ 𝒙 = 0
  
● [16:16]. Step 5: Create a table with the critical numbers and the inflection point.  
                                                            CP₁= -1                              CP₂ = 1
...
● [22:23]. Step 7: Plot the points on the Cartesian plane.
...
● [30:49]. Step 2: Asymptotes:
     ...
     ◉ Horizontal asymptotes: Take limits as 𝒙 tends to positive and negative infinity.
          ○ Divide numerator and denominator by the highest power in the denominator (𝒙³).
          ○ limₓ→ ∞ 𝒇(𝒙) = 0 and limₓ→ -∞) 𝒇(𝒙) = 0, so the horizontal asymptote is 𝑦 = 0.
    
● [37:44]. Step 3: First derivative test:
     ◉ Use the quotient rule to differentiate.
     ◉ Result: 𝒇'(𝒙) = (-𝒙⁴ + 3𝒙²) / 𝒙⁶ = (-𝒙² + 3) / 𝒙⁴
     ◉ [40:00]. Find critical numbers by setting 𝒇'(𝒙) = 0 and considering where 𝒇'(𝒙) is not defined.
          ○ 𝒙 = 0, 𝒙 = √3, and 𝒙 = -√3.
    
● [42:46]. Step 4: Second derivative test:
     ◉ Result: 𝒇''(𝒙) = (2𝒙² - 6) / 𝒙⁵
     ◉ [45:11]. Find possible inflection points by setting 𝒇''(𝒙) = 0 and considering where 𝒇''(𝒙) is not defined.
          ○ 𝒙 = 0, 𝒙 = √6, and 𝒙 = -√6.
   
● [47:14]. Step 5: Create a table with the critical numbers and the possible inflection points.
     ◉ Test values in each interval to determine the sign of the first and second derivatives.
                                                           CP₁ = -√3                             𝒙 = 0 (VA) 𝒇'(𝒙) is not defined      CP₂ = √3
         𝒇′(𝒙)              ±? inc/dec= -dec          ∣    ±?   inc/dec = +inc       ∣          ±? inc/dec = +inc               ∣    ±? inc/dec= -dec     ...  Include also VA's and HOLD's
                       ------------------------------------∣----------------------------------∣-------------------------------------------∣------------------------
        𝒇′′(𝒙)   ±? Conc.(Up/Down) = -down  ∣±? Conc.(Up/Down)=+up ∣ ±? Conc.(Up/Down) = -down    ∣  ±? Conc.(Up/Down)=+up
                                                           PIP₁ = - √6.                          𝒙 = 0  𝒇''(𝒙) is not defined              PIP₂ = √6                        ...  Include also VA's and HOLD's
● [57:31]. Step 6: Find the points.
...
● [58:15]. Step 7: Graph the function.
     ◉ Analyze intervals of increase and decrease alongside the asymptotes to sketch the graph.
     ◉ Conclude that the graph matches the information on concavity.



🧩 Ｅｘａｍｐｌｅ　𝒇（𝒙）　＝　（２𝒙²　－　８）　／　（𝒙²　－　１６）

● [1:05:25]. Step 1: Intercepts:
     ◉ 𝒙-intercepts: 2𝒙² - 8 = 0
          ○ 𝒙 = 2 and 𝒙 = -2
     ◉ 𝑦-intercept: 𝒇0）
          ○ (0, 1/2)
    
● [1:06:26]. Step 2: Asymptotes:
     ◉ Vertical asymptotes: 𝒙²－16 = 0
          ○ 𝒙 = 4 and 𝒙 = -4
     ◉ Horizontal asymptotes: lim_(𝒙 → ∞) 𝒇(𝒙) = 2 and lim_(𝒙 → -∞) 𝒇(𝒙) = 2
          ○ The horizontal asymptote is 𝑦 = 2.
    
● [1:08:22]. Step 3: First derivative test:
     ◉ 𝒇'(𝒙) = -48𝒙 / (𝒙² - 16)²
     ◉ Find critical numbers by setting 𝒇'(𝒙) = 0 (-48𝒙 = 0)
          ○ 𝒙 = 0
     ◉ Considering where 𝒇'(𝒙) is not defined:  (𝒙² - 16)² = 0
          ○ 𝒙 = 4, and 𝒙 = -4.
  
● [1:12:15]. Step 4: Second derivative test:
     ◉ 𝒇''(𝒙) = 48(3𝒙² + 16) / (𝒙² - 16)³
...
● [1:19:19]. Step 5: Create a table with the critical numbers and possible inflection points.
     ◉ Test values in each interval to determine the sign of the first and second derivatives.
                                                           𝒙  = -4 (VA)                          CP₁= 0                                           𝒙 = 4  (VA)
...
● [1:24:23]. Step 6: Determine the points to plot using previous information.  
     ◉ (2,0) (-2,0)
     ◉ R.MAX: (0, ½)
     ◉ R.MIN:❌
     ◉ IP's:❌

● [1:26:26]. Step 7: Graph the function.